...
L["Show the tooltip line even when the item is not in the database"] = "即使物品不在数据库中也显示鼠标提示"
L["Modifier keys"] = "组合键"
L["Display the tooltip only when the selected modifier keys being are pressed. " ..
    "(No selections means always show.)"] = "仅当选中的组合键被按下时显示鼠标提示（全不选即为始终显示）。"
...
L["Including community updates changes some items' version/expansion to the one " ..
            ...
            "reporting it to the project page."] = "使用社区修正的版本信息。" ..
            "例如[髓根草]在争霸艾泽拉斯最后的预发布版本中添加，但只能在暗影国度版本中获取。" ..
            "当启用该选项时，ItemVersion 会认为该物品添加自暗影国度而非争霸艾泽拉斯。\n\n" ..
